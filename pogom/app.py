...
from . import config
from .models import Pokemon, Gym, Pokestop, ScannedLocation, CurrentLocation, PokeAlert, Vars, LocLog, captcha
from .search import calc_distance
...
class Pogom(Flask):
    def __init__(self, import_name, **kwargs):
        ...
        self.route("/blank", methods=['GET'])(self.blank)
        self.route("/load_challenge", methods=['GET'])(self.load_challenge)
        self.route("/set_alert", methods=['GET'])(self.set_alert)
        self.route("/loc", methods=['GET'])(self.loc)
        self.route("/next_loc", methods=['POST'])(self.next_loc)
        self.route("/new_radius", methods=['GET'])(self.new_radius)
        self.route("/autolocate", methods=['GET'])(self.autolocate)
    ...
    def fullmap(self):
        ...
        if request.args.get('loc'):
            ...
            if not len(re.sub("[0-9\.\-]+", "", loc)):
                loc = re.sub("([0-9\.]+)([0-9\.\-])", "\g<1>,\g<2>", request.args.get('loc'), 1)
    ...
    def set_alert(self):
        d = {}
        pokemon_id = int(request.args.get('poke_id'))
        priority = int(request.args.get('priority'))
        PokeAlert.enable_alert(pokemon_id, priority)
        d['success'] = 1;
        log.info('Set alert for %d priority %d', pokemon_id, priority)
        return jsonify(d)
    ...
    def raw_data(self):
        ...
        d['scanradius'] = Vars.get("scanradius")
...
#            data = data.replace('window.location.href = "unity:".concat(str);', 'console.log(str)')
#            d['data'] = b64encode(data)
        return d['url'], 200
    ...
    def next_loc(self):
        ...
        if not (lat and lon):
            ...
            dist = calc_distance(self.current_location, (lat, lon, 0))
            if dist <= .07:
                log.info('Distance change (%f) not large enough to warrant a scan update', dist)
            else:
    ...
    def new_radius(self):
        ...
        if request.args:
            ...
            log.info("New radius set: %d", radius)
            Vars.set("scanradius", radius)
        return jsonify(['1'])

    def autolocate(self):
        args = get_args()
        if request.args:
            checked = request.args.get('autolocate')
            if checked == "true":
                Vars.set("autoloc", "1")
            else:
                Vars.set("autoloc", "0")
