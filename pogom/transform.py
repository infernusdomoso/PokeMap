...
def mults_for_loc(initial_loc):
    ...
    pulse_radius = .07         # km - radius of players heartbeat is 70m
    ...
    xoffset = round(abs(abs(initial_loc[1]) - abs(holder[1])), 5)
    holder = get_new_coords(initial_loc, ydist, NORTH)
    yoffset = round(abs(abs(initial_loc[0]) - abs(holder[0])), 6)
    if(initial_loc[0] < 0):
        yneg = -1
    if(initial_loc[1] < 0):
        xneg = -1
    ymult = math.floor(abs(initial_loc[0] / yoffset))
    xmult = math.floor(abs(initial_loc[1] / xoffset))
    xmp = 0
    if not (ymult % 2):
        xmp += .5
    locked_y = (ymult + .5) * yoffset * yneg
    locked_x = (xmult + xmp) * xoffset * xneg
    
    return (ymult, xmult, locked_y, locked_x)


def generate_location_steps(initial_loc, step_count, step_distance, type, direction=0):
    # Bearing (degrees)
    NORTH = 0
    EAST = 90
    SOUTH = 180
    ...
    mults = mults_for_loc(initial_loc)
    initial_loc = (mults[2], mults[3], 0)
    ...
    if type == "line":
        if step_count > 1:
            ...
            while ring < step_count:
                loc = get_new_coords(loc, xdist, direction)
                mults = mults_for_loc(loc)
                n_loc = (mults[2], mults[3], 0)
                results.append((n_loc[0], n_loc[1], 0))
                if n_loc[1] > loc[1]:
                    lloc = get_new_coords(n_loc, xdist, 90)
                    mults = mults_for_loc(lloc)
                    n_loc = (mults[2], mults[3], 0)
                    results.append((n_loc[0], n_loc[1], 0))
                elif n_loc[1] < loc[1]:
                    lloc = get_new_coords(n_loc, xdist, 180)
                    mults = mults_for_loc(lloc)
                    n_loc = (mults[2], mults[3], 0)
                    results.append((n_loc[0], n_loc[1], 0))
