...
from pogom.models import init_database, create_tables, drop_tables, Pokemon, db_updater, clean_db_loop, setLockPos,CurrentLocation
...
logging.basicConfig(format='%(asctime)s [%(threadName)10s][%(module)10s][%(levelname)8s] %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
def main():
    ...
    CurrentLocation.set_loc(position[0], position[1])
    ...
    if not args.only_server:
        ...
        search_thread = Thread(target=search_overseer_thread, name='srchMaster', args=argset)
